...
import { CheckSquare, Lock, Mail, ArrowRight, ShieldCheck, Sparkles } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      addToast('Welcome back to FlowBoard!');
      navigate('/');
    } catch (err) {
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 selection:bg-indigo-500 selection:text-white">
      <div className="w-full max-w-md space-y-8 bg-slate-900/90 border border-slate-800 p-8 rounded-3xl shadow-2xl backdrop-blur-xl">
        ...
        <div className="text-center space-y-2">
          <div className="w-14 h-14 bg-gradient-to-tr from-indigo-600 via-indigo-500 to-purple-500 rounded-2xl flex items-center justify-center mx-auto shadow-xl shadow-indigo-600/30">
            <CheckSquare className="w-7 h-7 text-white" />
          </div>
          <h2 className="text-2xl font-black text-white tracking-tight">FlowBoard</h2>
          <p className="text-xs text-slate-400 font-medium">Next-Gen Agile Project Management</p>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-indigo-950/30 border border-indigo-500/20 space-y-2.5">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              type="button"
              onClick={() => handleDemoLogin('elena@flowboard.dev', 'password123')}
            ...
            >
              <div className="font-bold text-indigo-300">Elena Rostova</div>
              <div className="text-[10px] text-slate-400">Product Manager</div>
            </button>
            <button
              type="button"
              onClick={() => handleDemoLogin('alex@flowboard.dev', 'password123')}
              className="px-2.5 py-2 rounded-xl bg-slate-800 hover:bg-indigo-600/20 border border-slate-700/80 hover:border-indigo-500/40 text-[11px] font-semibold text-slate-200 transition-colors text-left"
            >
              <div className="font-bold text-indigo-300">Alex Rivera</div>
              <div className="text-[10px] text-slate-400">Tech Lead</div>
            </button>
          </div>
        </div>
